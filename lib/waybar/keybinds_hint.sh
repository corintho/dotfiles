...
[ -f "$kb_cache" ] && { trap '$scrDir/keybinds.hint.py --format rofi > "$kb_cache" && echo "Keybind cache updated" ' EXIT; }

output="$(
  if ! cat "$kb_cache" 2>/dev/null; then
    $scrDir/keybinds.hint.py --format rofi | tee "$kb_cache"
  fi
)"
wait
if [ -z "$output" ]; then
...
selected=$(echo -e "$output" | rofi -dmenu -p "filter" \
