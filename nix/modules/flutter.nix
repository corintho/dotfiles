{ nixpkgs, local_flutter_path, flutter_version }:
...
in rec {
  ...
  flutter_source = if desired_version == "3.38.4" then
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.38.4-stable.zip";
    ...
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.24.4-stable.zip";
      hash = "sha256-SofEuMzDMKEvSSH9RY7xq5wkr3JiHQy78+ZRz5rDYkY=";
    }
  else if desired_version == "3.24.2" then
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.24.2-stable.zip";
      hash = "sha256-zmm9TyR2Mzi+zGcl0e2kqNnFPhvrwSl04y0qu5ojxnY=";
    }
  else if desired_version == "3.24.0" then
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.24.0-stable.zip";
      hash = "sha256-PEQ5hcnNpfaVmidMrd/rOOEXd/yO2Kaa5wChSF5t0lg=";
    }
  else if desired_version == "3.22.2" then
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.22.2-stable.zip";
      hash = "sha256-P7XWSTbisLMvdpPX0jf6nkcIy3AOjJWyx5rAhV5x2xE=";
    }
  else if desired_version == "3.22.0" then
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.22.0-stable.zip";
    ...
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.19.5-stable.zip";
      hash = "sha256-HXHsHs2bzt9Xaqp6cUyK/S/Qk028jqCfSx3DF31HX/Q=";
    }
  else if desired_version == "3.19.4" then
    pkgs.fetchurl {
      url =
        "https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.19.4-stable.zip";
  ...
  unpack_flutter = pkgs.writeShellApplication {
    name = "unpack_flutter";
    runtimeInputs = with pkgs; [ git unzip which ];
